...
/**
 * Strong modern 9.0 internals can show 9–10 upside when no limiter is present.
 * Does not change psaGrade / internalGrade. Wear, defects, or a weak pillar keep 8–9.
 */
function qualifiesForModernGemUpsideRange(analysis, era, categoryScores, internalGrade) {
  if (era !== "modern" || internalGrade !== 9) {
    return false;
  }

  const { corners, edges, surface, centering } = categoryScores;
  if (corners < 9 || edges < 9 || surface < 9 || centering < 9) {
    return false;
  }
  ...
  return (analysis.defects || []).length === 0;
}
...
export function computeGrade(analysis, era) {
  ...
  const likelyRange = qualifiesForModernGemUpsideRange(
    analysis,
    era,
    categoryScores,
    internalGrade
    ...
    ? "PSA 9–10"
    : formatLikelyRange(internalGrade);
