...
const NM_UNIFORM_LIGHT_GUARD_BLOCKERS = new Set([
  "moderate_crease",
  "severe_crease",
  "writing_mark",
  "writing_mark_severe",
...
/**
 * NM guard: skip uniform_light optimism ceiling when slab evidence supports NM.
 */
export function qualifiesForNmUniformLightGuard(categoryFloor, centering, defects) {
  if (categoryFloor == null || categoryFloor < 8) {
    return false;
  }
  if (centering == null || centering < 9) {
    return false;
  }
  if (countModeratePlusDefects(defects) > 0) {
    return false;
  ...
  return !defects.some((defect) =>
    NM_UNIFORM_LIGHT_GUARD_BLOCKERS.has(
      resolveEffectiveDefectTag(defect.tag, defect.severity)
    )
  );
}
