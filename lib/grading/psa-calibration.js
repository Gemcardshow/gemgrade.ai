...
export function applyVintageMultiPillarWearCap(
  ...
  const nmTriadCapSkip =
    nmCapSkip || qualifiesForVintageNmTriadCapSkip(categoryScores, defects, analysis);
...
export function applyVintageTriadSkipCategoryFloorRelief(
  ...
  const evidenceFloor = getWearFloor(evidence);
  ...
  if (evidenceFloor <= impactMin && evidenceFloor <= categoryFloor) {
    return categoryFloor;
  }

  const relieved = Math.max(categoryFloor, evidenceFloor);
