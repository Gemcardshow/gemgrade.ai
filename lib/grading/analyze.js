...
function captureScratchStage(trace, stage, snapshot) {
  if (!trace) {
    return;
  }
  recordScratchStage(trace, stage, snapshot);
}
...
const MODERN_GLOSSY_STRONG_SCRATCH_EVIDENCE = [
  /\b(linear|hairline) scratch/i,
  /\bscratch(es)? crossing (the )?(artwork|background|image|portrait|surface)/i,
...
/** @deprecated alias — use MODERN_GLOSSY_STRONG_SCRATCH_EVIDENCE */
const CHROMIUM_STRONG_SCRATCH_EVIDENCE = MODERN_GLOSSY_STRONG_SCRATCH_EVIDENCE;

const MODERN_GLOSSY_GENERIC_SCRATCH_LANGUAGE = [
  /\blight scratch present/i,
  /\bminor surface scratch/i,
  /\bsurface scratch noted/i,
  /\bsurface scratch found/i,
  /\bminor surface scratch noted/i,
  /\bminor scratch on (the )?(front|back) surface/i,
  /\bscratch on (the )?(front|back) surface/i,
  ...
  /\bscratch noted\b/i,
  /\bscratch found near/i,
  /\bscratch(es)? (found )?near (the )?(bottom|top|left|right|lower|upper)?\s*(edge|border)/i,
  /\baffecting surface quality\b/i,
  /\bminor surface mark/i,
];

/** @deprecated alias — use MODERN_GLOSSY_GENERIC_SCRATCH_LANGUAGE */
const CHROMIUM_GENERIC_SCRATCH_LANGUAGE = MODERN_GLOSSY_GENERIC_SCRATCH_LANGUAGE;
...
const MODERN_GLOSSY_FINISH_SIGNALS =
  /\b(bowman chrome|topps chrome|panini prizm|prizm silver|panini select|select chrome|mosaic|optic holo|refractor|chrome refractor|holo finish|chrome finish|zero gravity|holo insert|holographic|holographic finish|glossy|reflective finish|reflective stock|glossy stock|glossy modern)\b/i;

/** @deprecated alias — use MODERN_GLOSSY_FINISH_SIGNALS */
const CHROMIUM_FINISH_SIGNALS = MODERN_GLOSSY_FINISH_SIGNALS;

const MODERN_GLOSSY_EXPLICIT_SCRATCH_LOCATION =
  /\bscratch(es)?\s+(found\s+)?(on|at|near|along|in|across|through)\s+(the\s+)?(front|back|surface|artwork|background|portrait|image|left|right|upper|lower|center|border)/i;

/** @deprecated alias */
const CHROMIUM_EXPLICIT_SCRATCH_LOCATION = MODERN_GLOSSY_EXPLICIT_SCRATCH_LOCATION;

function surfaceNoteText(raw) {
  return String(raw?.categoryNotes?.surface || "");
}

function collectModernGlossyFinishText(raw) {
  return [
...
function isModernGlossyFinishCard(raw) {
  if (raw?.cardMeta?.isReflective === true) {
    return true;
  }
  return MODERN_GLOSSY_FINISH_SIGNALS.test(collectModernGlossyFinishText(raw));
}

/** @deprecated alias — use isModernGlossyFinishCard */
function isChromiumFinishCard(raw) {
  return isModernGlossyFinishCard(raw);
}
...
function hasModernGlossyStrongScratchEvidence(raw, defect = null) {
  const note = stripNegatedScratchLanguage(surfaceNoteText(raw));
  if (!note) {
    return false;
  }
  if (MODERN_GLOSSY_STRONG_SCRATCH_EVIDENCE.some((pattern) => pattern.test(note))) {
    return true;
  }
  return (
    defect?.confidence === "high" &&
    MODERN_GLOSSY_EXPLICIT_SCRATCH_LOCATION.test(note)
  );
}

/** @deprecated alias */
function hasChromiumStrongScratchEvidence(raw, defect = null) {
  return hasModernGlossyStrongScratchEvidence(raw, defect);
}

function isModernGlossyGenericScratchOnly(raw) {
  const note = stripNegatedScratchLanguage(surfaceNoteText(raw));
  ...
  const hasStrong = hasModernGlossyStrongScratchEvidence(raw);
  if (hasStrong) {
    return false;
  }
  return MODERN_GLOSSY_GENERIC_SCRATCH_LANGUAGE.some((pattern) => pattern.test(text));
}

/** @deprecated alias */
function isChromiumGenericScratchOnly(raw) {
  return isModernGlossyGenericScratchOnly(raw);
}

const VINTAGE_NM_STRONG_SCRATCH_EVIDENCE = [
  ...MODERN_GLOSSY_STRONG_SCRATCH_EVIDENCE,
  /\b(visible|clear)\s+scratch(es)?\b/i,
  /\bscratch(es)?\s+found\s+on\s+(the\s+)?(front|back)\b/i,
  /\bscratch(es)?\s+found\s+on\s+(the\s+)?(front|back)\s+surface\b/i,
];

const VINTAGE_NM_GENERIC_SCRATCH_LANGUAGE = [
  /\bfew light scratches?\b/i,
  /\bminor surface scratches?\b/i,
  /\bclean surface with (a )?(few )?minor scratches?\b/i,
  /\blight scratching consistent with age\b/i,
  /\bgeneral surface wear\b/i,
  /\bsuperficial marks?\b/i,
  /\bslight surface scratches?\b/i,
  /\ba few light scratches?\b/i,
  /\bfew minor scratches?\b/i,
  /\bminor scratches?( present| observed| noted|,|\s)/i,
  /\blight scratches detected;\s*otherwise clean\b/i,
  /\blight scratches;\s*otherwise clean\b/i,
  /\b(do not detract|not detract significantly|not affecting (overall )?presentation)\b/i,
  /\bclean surface with (a )?few\b/i,
  /\bgenerally clean surface with (a )?few light scratches?\b/i,
  /\bminor surface scratch(es)? observed\b/i,
  /\blight scratches detected\b/i,
  ...MODERN_GLOSSY_GENERIC_SCRATCH_LANGUAGE,
];

function hasVintageNmStrongScratchEvidence(raw, defect = null) {
  const note = stripNegatedScratchLanguage(surfaceNoteText(raw));
  const presentation = stripNegatedScratchLanguage(collectSurfaceScratchText(raw));
  const text = [note, presentation].filter(Boolean).join(" ");
  if (!text) {
  ...
  if (VINTAGE_NM_STRONG_SCRATCH_EVIDENCE.some((pattern) => pattern.test(text))) {
    return true;
  }
  return (
    defect?.confidence === "high" &&
...
function noteIndicatesModeratePlusSurfaceDamage(raw) {
  const text = collectSurfaceScratchText(raw);
  return /\b(moderate scratch|continuous scratch|deep scratch|heavy scratch|moderate scratching|scratching visible)\b/i.test(
    text
  );
}

function qualifiesForVintageNmScratchSkepticism(categoryScores, raw, defects) {
  if (noteIndicatesModeratePlusSurfaceDamage(raw)) {
    return false;
  }
  if (raw.__visionDefectTagsAtEntry?.includes("surface_wear")) {
    return false;
  }
  if (countNotesPillarsWithPoorWear(raw) >= 2) {
    return false;
  }
  if (defects.some((defect) => defect.tag === "surface_scratch_moderate")) {
    return false;
  }
  const { corners, edges, surface } = categoryScores;
  const wearMin = Math.min(corners, edges, surface);
  if (wearMin < 6.5) {
    return false;
  }
  const pillarsAt7 = [corners, edges, surface].filter((score) => score >= 7).length;
  return pillarsAt7 >= 2;
}

function hasConfirmedVintageNmSurfaceScratchEvidence(raw, defect) {
  ...
  if (
    ...
    !hasVintageNmStrongScratchEvidence(raw, defect)
  ) {
    return false;
  }

  if (!hasVintageNmStrongScratchEvidence(raw, defect)) {
    if (defect?.confidence === "high") {
      const text = stripNegatedScratchLanguage(collectSurfaceScratchText(raw));
      if (
        /\bscratch/i.test(text) &&
        !VINTAGE_NM_GENERIC_SCRATCH_LANGUAGE.some((pattern) => pattern.test(text))
      ) {
        return true;
      }
    }
  ...
  if (
    /\b(print line|roller mark|factory line)\b/i.test(surfaceNote) &&
...
function shouldBlockStripForRyanIsolation(defects, categoryScores, raw, defect) {
  if (hasConfirmedVintageNmSurfaceScratchEvidence(raw, defect)) {
    return false;
  }
  const candidate = defects.filter((entry) => entry.tag !== "surface_scratch_light");
  if (candidate.length !== 1 || candidate[0]?.tag !== "corner_wear_light") {
    return false;
  }
  const { corners, edges, surface } = categoryScores;
...
function reconcileVintageNmScratchSkepticism(defects, raw, categoryScores) {
  if (!defects.some((defect) => defect.tag === "surface_scratch_light")) {
    return { defects, audit: [], reconciled: false };
  }
  if (!qualifiesForVintageNmScratchSkepticism(categoryScores, raw, defects)) {
    return { defects, audit: [], reconciled: false };
  }
  ...
  const reconciled = defects.filter((defect) => {
    ...
    if (shouldBlockStripForRyanIsolation(defects, categoryScores, raw, defect)) {
      return true;
    }
    if (hasConfirmedVintageNmSurfaceScratchEvidence(raw, defect)) {
      return true;
    ...
    audit.push({
      source: "vintage_nm_scratch_skepticism_strip",
...
function hasExplicitSurfaceScratchInNotes(raw, defect = null) {
  ...
  if (isModernGlossyFinishCard(raw)) {
    if (isModernGlossyGenericScratchOnly(raw)) {
      return false;
    }
    return hasModernGlossyStrongScratchEvidence(raw, defect);
  }
...
function hasConfirmedSurfaceScratchEvidence(raw, defect) {
  ...
  if (isModernGlossyFinishCard(raw) && isModernGlossyGenericScratchOnly(raw)) {
    return false;
  }
...
const MODERN_NEGATED_PILLAR_DAMAGE_TERMS =
  "whitening|chipping|fraying|roughness|layering|peeling|nicks?|dings?|damage|wear|issues|flaws|imperfections|rounding";

function stripNegatedPillarDamageLanguage(note) {
  if (!note) {
    return "";
  }
  const damage = MODERN_NEGATED_PILLAR_DAMAGE_TERMS;
  const negatedList = new RegExp(
    `\\b(?:no|without|free of|lack of)\\s+(?:(?:visible|signs?\\s+of|evidence\\s+of|notable)\\s+)?(?:${damage})(?:(?:\\s*,\\s*(?:or\\s+)?|\\s+or\\s+)(?:${damage}))*\\b`,
    "gi"
  );
  const showingNegatedList = new RegExp(
    `\\b(?:showing|with)\\s+(?:no|without)\\s+(?:(?:signs?\\s+of|evidence\\s+of)\\s+)?(?:${damage})(?:(?:\\s*,\\s*(?:or\\s+)?|\\s+or\\s+)(?:${damage}))*\\b`,
    "gi"
  );
  return note
    .replace(negatedList, "")
    .replace(showingNegatedList, "")
    .replace(
    ...
    .replace(
      new RegExp(
        `\\b(no|without)\\s+(chipping|fraying|whitening|roughness|damage|wear)\\s+or\\s+(chipping|fraying|whitening|roughness|damage|wear)\\b`,
        "gi"
      ),
      ""
    )
    .replace(
      /\b(no|without)\s+(visible|noticeable|significant|obvious|notable)\s+(wear|fraying|chipping|roughness|whitening|damage|issues|flaws)\b/gi,
      ""
    )
    .replace(/\b(no|without)\s+edge damage\b/gi, "")
    .replace(new RegExp(`\\s*,\\s*(?:or\\s+)?(?:${damage})\\b`, "gi"), "")
    .replace(new RegExp(`\\s+or\\s+(?:${damage})\\b`, "gi"), "");
}
...
function hasModernSurfaceReconcileBlockers(note, raw = null) {
  ...
  if (raw && isModernGlossyFinishCard(raw)) {
    if (
      hasModernGlossyStrongScratchEvidence(
...
export function hasNmGemPresentationAppeal(raw) {
  const appeal = collectAppealText(raw).toLowerCase();
  return /\b(sharp corners|primarily sharp|well.?preserved|well preserved|clean surface|minimal wear|minimal visible wear|near.?mint|nm condition|strong color and|good imprint|intact corners|good eye appeal|good overall presentation|good overall appeal)\b/.test(
    appeal
  );
}
...
function hasVintageTrueStainOrMoistureEvidence(raw, defect) {
  const normalized = `${collectHarshConditionText(raw)} ${collectAppealText(raw)}`.toLowerCase();
  if (
    /\b(water damage|water stain|moisture|humidity|warped|warping|mildew|mold)\b/.test(
      normalized
    )
  ) {
    return true;
  ...
  if (
    /\b(blotch|blotching|residue|paper loss|stock loss|oil stain|localized contamination|dark blotch)\b/.test(
      normalized
    )
  ) {
    return true;
  }
  if (/\b(heavy stain|heavy staining|extensive stain|large stain|significant stain)\b/.test(normalized)) {
    return true;
  }
  if (/\b(severe foxing|heavy foxing|extensive foxing)\b/.test(normalized)) {
    return true;
  }
  const location = String(defect.location || "").toLowerCase();
  if (location === "front") {
    return /\b(stain|discoloration)\b/.test(normalized);
  }
  if (location === "both") {
    return (
      /\b(front.*stain|stain.*front|front.*discoloration)\b/.test(normalized) &&
      /\b(moderate|heavy|visible|significant|limits)\b/.test(normalized)
    );
  }
  return false;
}

function hasVintageCosmeticBackToningEvidence(raw, defect) {
  const location = String(defect.location || "").toLowerCase();
  if (location === "front") {
    return false;
  }
  const normalized = `${collectHarshConditionText(raw)} ${collectAppealText(raw)}`.toLowerCase();
  if (/\bfoxing\b/.test(normalized)) {
    return false;
  }
  if (
    /\b(yellow back|yellowed back|yellow-back|cream back|off.?white back|normal vintage discoloration|age toning|paper tone|back toning|factory toning|stock oxidation|wax.?era discoloration)\b/.test(
      normalized
    )
  ) {
    return true;
  }
  if (hasVintagePaperVariationSignals(raw)) {
    return true;
  }
  const stainText = `${raw.categoryNotes?.surface || ""} ${raw.categoryNotes?.back || ""}`;
  if (
    noteIndicatesLightBackStain(stainText) &&
    !/\b(moisture|water|blotch|residue|paper loss|heavy|extensive)\b/.test(stainText.toLowerCase())
  ) {
    return true;
  }
  return false;
...
function hasVintageImplicitCosmeticBackStain(raw, categoryScores, defects, defect) {
  const location = String(defect.location || "").toLowerCase();
  if (location !== "back") {
    return false;
  }
  if (
    defects.some(
      (entry) =>
        (entry.tag === "staining_light" || entry.tag === "heavy_staining") &&
        entry.location === "front"
    )
  ) {
    return false;
  }
  const wearFloor = Math.min(
    categoryScores.corners,
    categoryScores.edges,
    categoryScores.surface
  );
  if (wearFloor < 5.5 || categoryScores.centering < 7) {
    return false;
  }
  const surfaceNote = String(raw.categoryNotes?.surface || "").toLowerCase();
  if (
    /\b(back stain|staining limits|discoloration limits|heavy stain|water|moisture|blotch|residue|paper loss)\b/.test(
      surfaceNote
    )
  ) {
    return false;
  ...
  return defects.every(
    (entry) => LIGHT_WEAR_ONLY_TAGS.has(entry.tag) || entry.tag === "staining_light"
  );
}

export function hasDistributedMultiPillarWearAppeal(raw, defects) {
  const lightFrontWearCount = (defects || []).filter(
    (defect) =>
      LIGHT_WEAR_ONLY_TAGS.has(defect.tag) &&
      (defect.location === "front" || defect.location === "both")
  ).length;
  if (lightFrontWearCount < 3) {
    return false;
  }
  const appeal = collectAppealText(raw).toLowerCase();
  return (
    /\bwear\b/.test(appeal) &&
    ["corner", "edge", "surface"].filter((pillar) => appeal.includes(pillar)).length >= 2
  );
}

function qualifiesForVintageCosmeticStainDemotion(raw, categoryScores, defects, defect, era = "vintage") {
  if (era !== "vintage" || defect.tag !== "staining_light") {
    return false;
  }
  if (hasVintageTrueStainOrMoistureEvidence(raw, defect)) {
    return false;
  }
  if (
    !hasVintageCosmeticBackToningEvidence(raw, defect) &&
    hasDistributedMultiPillarWearAppeal(raw, defects)
  ) {
    return false;
  }
  const wearFloor = Math.min(
  ...
  if (wearFloor <= 4.5 && countNotesPillarsWithPoorWear(raw) >= 2) {
    return false;
  }
  if (
    defects.some((entry) =>
  ...
  if (categoryScores.centering < 6.5 || wearFloor < 5.5) {
    return false;
  }
  return (
    hasVintageCosmeticBackToningEvidence(raw, defect) ||
    hasVintageImplicitCosmeticBackStain(raw, categoryScores, defects, defect)
  );
}

function hasBackStainDefectsForRelief(defects) {
  return (defects || []).some(
    (defect) =>
      (defect.tag === "staining_light" || defect.tag === "heavy_staining") &&
      defect.location === "back"
  );
}

function hasOnlyLightFrontWearForRelief(defects) {
...
function wouldReceiveVintageGemStainRelief(raw, categoryScores, defects) {
  if (!hasBackStainDefectsForRelief(defects)) {
    return false;
  }
  return (
    hasOnlyLightFrontWearForRelief(defects) &&
    categoryScores.centering >= 8 &&
    isNmVintagePresentationCandidate(categoryScores, raw)
  );
}

function wouldReceiveVintageExBackStainOnlyCeiling(raw, categoryScores, defects) {
  if (!hasBackStainDefectsForRelief(defects)) {
    return false;
  }
  if (categoryScores.centering < 7) {
  ...
  return hasOnlyLightFrontWearForRelief(defects);
}

function reconcileVintageCosmeticBackStaining(defects, categoryScores, raw, era = "vintage") {
  if (era !== "vintage") {
    return {
      defects,
      categoryScores,
      reconciled: false,
      vintageCosmeticBackStainRelief: false,
      vintageExBackStainOnlyReconciled: false,
    };
  }
  let adjusted = false;
  let vintageCosmeticBackStainRelief = false;
  let vintageExBackStainOnlyReconciled = false;
  const reconciled = (defects || []).flatMap((defect) => {
    if (qualifiesForVintageCosmeticStainDemotion(raw, categoryScores, defects, defect, era)) {
      adjusted = true;
      if (wouldReceiveVintageGemStainRelief(raw, categoryScores, defects)) {
        vintageCosmeticBackStainRelief = true;
      }
      if (wouldReceiveVintageExBackStainOnlyCeiling(raw, categoryScores, defects)) {
        vintageExBackStainOnlyReconciled = true;
      }
      return [];
    }
    return [defect];
  ...
  return {
    ...
    categoryScores,
    reconciled: true,
    vintageCosmeticBackStainRelief,
    vintageExBackStainOnlyReconciled,
  };
}

function shouldDemoteCosmeticStaining(defect, raw, categoryScores, defects = [], era = "vintage") {
...
function reconcileModernReflectiveScratchArtifacts(defects, raw, era) {
  if (era !== "modern" || !isModernGlossyFinishCard(raw)) {
    return { defects, audit: [], reconciled: false };
  }
  if (!defects.some((defect) => defect.tag === "surface_scratch_light")) {
    return { defects, audit: [], reconciled: false };
  ...
  if (
    hasExplicitSurfaceScratchInNotes(raw) &&
    MODERN_GLOSSY_STRONG_SCRATCH_EVIDENCE.some((pattern) => pattern.test(strippedSurfaceNote))
  ) {
    return { defects, audit: [], reconciled: false };
  }
...
function hasWritingStackDefects(defects) {
  return (defects || []).some((defect) =>
    ["writing_mark", "writing_mark_severe"].includes(defect.tag)
  );
}

function getWearFloorFromScores(categoryScores) {
  return Math.min(
    categoryScores.corners,
    categoryScores.edges,
    categoryScores.surface
...
function blocksNmTriadSkipForExSlab(visionBandScores, raw) {
  const { corners, edges, surface } = visionBandScores;
  const wearFloor = getWearFloorFromScores(visionBandScores);
  const maxPillar = Math.max(corners, edges, surface);

  if (wearFloor < 6) {
    return true;
  }

  if (maxPillar <= 6.5 && wearFloor >= 5.5) {
    return true;
  }
  ...
  if (
    ...
    corners >= 7 &&
    edges >= 7 &&
    !isNmVintagePresentationCandidate(visionBandScores, raw) &&
    !hasNmGemPresentationAppeal(raw) &&
    !isNmVintageCleanPresentation(visionBandScores, raw)
  ) {
    return true;
  }
...
export function qualifiesForVintageNmTriadBandGate(
  visionBandScores,
  raw,
  vintageCosmeticBackStainRelief = false,
  defects = null
) {
  if (vintageCosmeticBackStainRelief) {
    return true;
  }
  if (hasNmGemPresentationAppeal(raw)) {
    return true;
  }
  if (isNmVintageCleanPresentation(visionBandScores, raw)) {
    return true;
  }
  if (isNmVintagePresentationCandidate(visionBandScores, raw)) {
    return true;
  }

  const { corners, edges, surface, centering } = visionBandScores;
  const wearFloor = Math.min(corners, edges, surface);
  if (centering < 7 || wearFloor < 6.5) {
    return false;
  }

  const defectList = defects || raw.defects || [];
  if (
    defectList.some(
      (defect) =>
        defect.tag === "staining_light" &&
  ...
  if (wearFloor < 7 && Math.max(corners, edges, surface) >= 7) {
    const appeal = collectAppealText(raw);
    if (/\bpresents well\b/.test(appeal)) {
      return true;
    }
    return (
      hasNmGemPresentationAppeal(raw) ||
...
function hasBlockingMajorDefectForTriadSkip(defects) {
  return (defects || []).some(
    (defect) =>
      HIGH_GRADE_MAJOR_DEFECT_TAGS.has(defect.tag) &&
...
function defectsAllowNmTriadSkip(defects, visionAllLightWear) {
  if (defectsAreHighGradeLightWearOnly(defects)) {
    return true;
  }
  if (!visionAllLightWear) {
    return false;
  }
  return (defects || []).every(
    (defect) =>
...
export function qualifiesForHighGradeTriadSkip(categoryScores, raw, defects, options = {}) {
  const visionBandScores = options.visionBandScores || categoryScores;
  const vintageCosmeticBackStainRelief = Boolean(options.vintageCosmeticBackStainRelief);
  const visionAllLightWear = Boolean(options.visionAllLightWear);

  if (qualifiesForGemMintSlabProfile(defects, categoryScores, raw)) {
    return true;
  }
  if (!defectsAllowNmTriadSkip(defects, visionAllLightWear)) {
    return false;
  }
  if (hasBlockingMajorDefectForTriadSkip(defects)) {
    return false;
  }
  if (hasWritingStackDefects(defects)) {
    return false;
  }

  const visionWearFloor = getWearFloorFromScores(visionBandScores);
  if (visionWearFloor < 6) {
    return false;
  }
  if (hasDistributedMultiPillarWearAppeal(raw, defects)) {
    const bandGateEligible = qualifiesForVintageNmTriadBandGate(
      visionBandScores,
      raw,
      vintageCosmeticBackStainRelief,
      defects
  ...
  if (blocksNmTriadSkipForExSlab(visionBandScores, raw)) {
    return false;
  }
...
function reconcileHighGradeNmGemVisionCalibration(defects, categoryScores, raw) {
  ...
  let reconciled = (defects || []).flatMap((defect) => {
    if (shouldDemoteCosmeticStaining(defect, raw, categoryScores, defects, "vintage")) {
      adjusted = true;
      return [];
    }
...
function normalizeAnalysis(raw, era, options = {}) {
  const scratchTrace = options.scratchDiagnostics || null;
  if (!raw.__visionDefectTagsAtEntry) {
    raw = {
      ...raw,
      __visionDefectTagsAtEntry: (raw.defects || []).map((defect) => defect.tag),
    };
  }
  if (scratchTrace) {
    captureScratchStage(scratchTrace, "normalize_entry", {
      defects: raw.defects || [],
  ...
  if (era === "vintage") {
    ...
    const cosmeticBackStain = reconcileVintageCosmeticBackStaining(
      initialDefects,
      categoryScores,
      raw,
      era
    );
    if (cosmeticBackStain.reconciled) {
      stainWritingReconciled = true;
      vintageCosmeticBackStainRelief =
        vintageCosmeticBackStainRelief || cosmeticBackStain.vintageCosmeticBackStainRelief;
      vintageExBackStainOnlyReconciled =
        vintageExBackStainOnlyReconciled || cosmeticBackStain.vintageExBackStainOnlyReconciled;
      initialDefects = cosmeticBackStain.defects;
      categoryScores = cosmeticBackStain.categoryScores;
    }
  ...
  if (scratchTrace && era === "modern") {
    captureScratchStage(scratchTrace, "after_structural_infer", {
      defects: structuralDeduped,
      categoryNotes,
      raw,
      categoryScores,
    });
  ...
  if (scratchTrace) {
    captureScratchStage(scratchTrace, "after_primary_limiter_enforcement", {
      defects,
      categoryNotes,
      raw,
      finalLimiter,
  ...
  const triadSkip = qualifiesForHighGradeTriadSkip(categoryScores, raw, defects, {
      visionBandScores: writingReliefBandScores,
      vintageCosmeticBackStainRelief,
      visionAllLightWear,
    });
  const triadProfile =
    ...
    countNotesPillarsWithWear(raw) >= 3 &&
    !triadSkip;
  let vintageTriadNormalizeClamp = false;
  let preTriadClampWearScores = null;
  ...
  if (triadClamp.reconciled) {
    vintageTriadNormalizeClamp = true;
    preTriadClampWearScores = {
      corners: categoryScores.corners,
  ...
  if (era === "vintage") {
    const vintageNmScratch = reconcileVintageNmScratchSkepticism(
      defects,
      raw,
      writingReliefBandScores
    );
    if (vintageNmScratch.reconciled) {
      defects = vintageNmScratch.defects;
      visionReconciliationAudit = [
        ...visionReconciliationAudit,
        ...vintageNmScratch.audit,
      ];
      finalLimiter = resolvePrimaryLimiter(
        defects,
        era,
  ...
  if (scratchTrace && era === "modern") {
    captureScratchStage(scratchTrace, "before_chromium_scratch_reconcile", {
      defects,
      categoryNotes,
      raw,
      finalLimiter,
  ...
  if (scratchTrace && era === "modern") {
    captureScratchStage(scratchTrace, "after_chromium_scratch_reconcile", {
      defects,
      categoryNotes,
      raw,
      finalLimiter,
  ...
  if (scratchTrace && era === "modern") {
    captureScratchStage(scratchTrace, "after_finalize_surface_scratch", {
      defects,
      categoryNotes,
      raw,
      finalLimiter,
  ...
  if (scratchTrace && era === "modern") {
    captureScratchStage(scratchTrace, "after_handling_wear_pillar_reconcile", {
      defects,
      categoryNotes,
      raw,
      finalLimiter,
  ...
  if (scratchTrace) {
    captureScratchStage(scratchTrace, "normalize_complete", {
      defects,
      categoryNotes,
      raw,
      finalLimiter,
 ...
 */
export async function analyzeCard(client, { frontImage, backImage, era }, options = {}) {
  ...
  const traceEnabled = isScratchDiagnosticsEnabled(options);
  const scratchDiagnostics = traceEnabled ? createScratchDiagnosticTrace(raw) : null;
  const analysis = normalizeAnalysis(raw, era, { scratchDiagnostics });

  if (traceEnabled) {
    return { analysis, rawVision: raw, scratchDiagnostics };
  }
