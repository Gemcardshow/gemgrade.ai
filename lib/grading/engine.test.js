...
import {
  ...
  applyVintageExSlabBandRecovery,
  triggersPsa1Calibration,
  qualifiesForNmBandVintageCapSkip,
  qualifiesForNmUniformLightGuard,
...
test("formatLikelyRange keeps generic integer 9 as PSA 8–9", () => {
  assert.equal(formatLikelyRange(9), "PSA 8–9");
  assert.equal(formatLikelyRange(9.0), "PSA 8–9");
  assert.equal(formatLikelyRange(9.5), "PSA 9–10");
});

test("modern 9/9/9.5/9 with no limiter uses 9-10 confidence range without changing GemGrade", () => {
  const analysis = baseAnalysis({
    categoryScores: { corners: 9, edges: 9, surface: 9.5, centering: 9 },
    defects: [],
  ...
  assert.equal(result.likelyRange, "PSA 9–10");
  assert.equal(result.primaryLimiter.tag, null);
  assert.equal(result.primaryLimiter.label, "None visible");
});

test("modern 9/9/9/9 with no limiter is also eligible for 9-10 confidence range", () => {
  const analysis = baseAnalysis({
    categoryScores: { corners: 9, edges: 9, surface: 9, centering: 9 },
    defects: [],
    primaryLimiterTag: null,
...
test("modern 9.5 internal still uses formula range 9-10 and headline 9", () => {
  const analysis = baseAnalysis({
    categoryScores: { corners: 9.5, edges: 9.5, surface: 9.5, centering: 9.5 },
    defects: [],
    primaryLimiterTag: null,
    primaryLimiterLabel: "None visible",
    scanQuality: { level: "excellent", visibilityIssues: [], inspectionLimits: [] },
  ...
  const result = computeGrade(analysis, "modern");
  assert.equal(result.internalGrade, 9.5);
  assert.equal(result.psaGrade, 9);
  assert.equal(result.likelyRange, "PSA 9–10");
});

test("vintage 9/9/9.5/9 with no limiter keeps 8-9 confidence range", () => {
  const analysis = baseAnalysis({
    categoryScores: { corners: 9, edges: 9, surface: 9.5, centering: 9 },
    defects: [],
    primaryLimiterTag: null,
    primaryLimiterLabel: "None visible",
    scanQuality: { level: "excellent", visibilityIssues: [], inspectionLimits: [] },
    cardMeta: { estimatedYear: 1987, isReflective: false, isDarkBorder: false },
  });

  const result = computeGrade(analysis, "vintage");
  assert.equal(result.psaGrade, 9);
  assert.equal(result.internalGrade, 9);
  assert.equal(result.likelyRange, "PSA 8–9");
});

test("modern GemGrade 9 with weak centering keeps 8-9 confidence range", () => {
  const analysis = baseAnalysis({
    categoryScores: { corners: 9, edges: 9, surface: 9.5, centering: 8.5 },
    defects: [],
    primaryLimiterTag: null,
    primaryLimiterLabel: "None visible",
    scanQuality: { level: "excellent", visibilityIssues: [], inspectionLimits: [] },
...
test("modern GemGrade 9 with cosmetic print_line limiter keeps 8-9 confidence range", () => {
  const analysis = baseAnalysis({
    categoryScores: { corners: 9, edges: 9, surface: 9, centering: 9 },
    defects: [
      {
    ...
    primaryLimiterLabel: "Print line or roller mark",
    categoryNotes: {
      corners: "Sharp corners with no visible wear.",
      edges: "Clean edges.",
...
test("modern GemGrade 9 with surface scratch limiter keeps 8-9 confidence range", () => {
  const analysis = baseAnalysis({
    categoryScores: { corners: 9, edges: 9, surface: 9, centering: 9 },
    defects: [
      {
...
test("modern GemGrade 9 with leftover limiter tag keeps 8-9 confidence range", () => {
  const analysis = baseAnalysis({
    categoryScores: { corners: 9, edges: 9, surface: 9.5, centering: 9 },
    defects: [],
    primaryLimiterTag: "corner_wear_light",
...
test("modern handling-wear cap does not receive 9-10 confidence range", () => {
  ...
  assert.notEqual(result.likelyRange, "PSA 9–10");
});
...
test("vintage Ryan optimism ceiling still caps single corner_wear_light inflation", () => {
  const analysis = baseAnalysis({
    categoryScores: { corners: 9, edges: 8.5, surface: 8.5, centering: 8 },
    defects: [
      {
        tag: "corner_wear_light",
        severity: "minor",
...
// vintage calibration phase 3 — Phase 3B-2: ex_slab_band_recovery evidence lift

test("F3B2-1 Rose PSA 9 — evidence recovery target >= 7.5 when recovery binds", () => {
  const { analysis, result } = gradeVintageSnapshot("1969-t-rose-psa9");
  assert.ok(result.psaGrade >= 8);
  assert.ok(Math.abs(result.psaGrade - 9) <= 1);
  const recovery = result.capAudit.find(
    (entry) => entry.source === "vintage:ex_slab_band_recovery"
  );
  if (recovery) {
    assert.ok((recovery.floor ?? 0) >= 7.5);
  }
  assert.ok(analysis.vintageTriadNormalizeClamp);
});

test("F3B2-2 Clemens PSA 9 — evidence recovery path improves deflated grade", () => {
  const { result } = gradeVintageSnapshot("1984-f-clemens-psa9");
  assert.ok(result.psaGrade >= 7);
  const recovery = result.capAudit.find(
    (entry) => entry.source === "vintage:ex_slab_band_recovery"
...
test("F3B2-3 Hunter / Drysdale PSA 9 — recovery evidence floor >= 7.5 when present", () => {
  for (const id of ["1967-t-hunter-psa9", "1968-t-drysdale-psa9"]) {
    const gradeFn = id.includes("hunter") ? gradeVintageCache : gradeVintageSnapshot;
    const { result } = gradeFn(id);
    assert.ok(result.psaGrade >= 8, `${id} expected Gem >= 8`);
    assert.ok(Math.abs(result.psaGrade - 9) <= 1, `${id} within ±1`);
    const recovery = result.capAudit.find(
      (entry) => entry.source === "vintage:ex_slab_band_recovery"
    );
    if (recovery) {
      assert.ok((recovery.floor ?? 0) >= 7.5, `${id} recovery floor`);
    }
  }
});

test("F3B2-4 synthetic — evidence recovery lifts crushed overall above legacy cap 6", () => {
  const categoryScores = { corners: 5.5, edges: 5.5, surface: 5.5, centering: 8 };
  const defects = [
    { tag: "corner_wear_light", severity: "minor", location: "front", confidence: "high" },
    { tag: "edge_wear_light", severity: "minor", location: "front", confidence: "high" },
  ];
  const analysis = {
    ...
    preTriadClampWearScores: { corners: 7, edges: 7, surface: 7.5, centering: 8 },
    visionCategoryScores: { corners: 7, edges: 7, surface: 7.5, centering: 8 },
    categoryScores,
    categoryNotes: {
      corners: "Light corner wear only.",
      edges: "Minor edge wear.",
      surface: "Clean surface with good overall eye appeal.",
      centering: "Well centered.",
    },
    eyeAppealSummary: "Good overall eye appeal with light wear only.",
    bestAttribute: "Strong centering",
    defects,
  };
  const capAudit = [];
  const lifted = applyVintageExSlabBandRecovery(
    5.5,
    categoryScores,
    defects,
    capAudit,
  ...
  assert.ok(lifted >= 7.5);
  const recovery = capAudit.find(
    (entry) => entry.source === "vintage:ex_slab_band_recovery"
  );
  assert.ok(recovery);
  assert.ok((recovery.floor ?? 0) >= 7.5);
...
test("F3B2-N1 Seaver PSA 9 — no evidence recovery lift", () => {
  const { result } = gradeVintageCache("1983-t-seaver-psa9");
  assert.ok(result.psaGrade <= 5);
  const recovery = result.capAudit.find(
    (entry) => entry.source === "vintage:ex_slab_band_recovery"
  ...
  if (recovery) {
    assert.ok((recovery.floor ?? 0) <= 6);
  }
});

test("F3B2-N2 Carew / Brydge — no evidence recovery inflation", () => {
  const { result: carew } = gradeVintageSnapshot("1967-t-carew-psa7");
  assert.equal(carew.psaGrade, 2);
  assert.ok(
    !carew.capAudit.some(
      (entry) =>
        entry.source === "vintage:ex_slab_band_recovery" &&
        (entry.floor ?? 0) >= 7
    )
  );
  const { result: brydge } = gradeVintageCache("1933-w-brydge-psa8");
...
test("F3B2-N3 Bird PSA 4 — 3E guard preserved under recovery lift", () => {
  const { result } = gradeVintageCache("1989-f-bird-psa4");
  assert.ok(result.psaGrade <= 5);
  assert.ok(
    !result.capAudit.some((entry) => entry.source === "nm_band:mint_floor")
...
test("F3B2-N4 Cobb / McCovey — writing stacks unchanged", () => {
  const { result: cobb } = gradeVintageCache("1950-c-cobb-psa9");
  assert.ok(cobb.psaGrade <= 5);
  const { result: mccovey } = gradeVintageCache("1980-t-mccovey-psa4");
  assert.ok(mccovey.psaGrade <= 2);
});

test("F3B2-N5 Yount PSA 9 — 3A regression unchanged", () => {
  const { result } = gradeVintageCache("1976-t-yount-psa9");
  assert.ok(result.psaGrade >= 8);
  assert.ok(Math.abs(result.psaGrade - 9) <= 1);
});
...
test("NM guard — Wright-style NM card bypasses uniform_light optimism ceiling", () => {
  const { result } = gradeVintageSnapshot("1975-t-wright-psa8");
  assert.ok(result.psaGrade >= 8);
  assert.ok(
    !result.capAudit.some(
      (entry) => entry.source === "ex_band:uniform_light_optimism_ceiling"
...
test("NM guard — Boggs-style NM card bypasses uniform_light optimism ceiling", () => {
  const { result } = gradeVintageCache("1983-t-boggs-psa7");
  assert.ok(result.psaGrade >= 8);
  assert.ok(
    !result.capAudit.some(
      (entry) => entry.source === "ex_band:uniform_light_optimism_ceiling"
...
test("NM guard — Bird PSA 4 stays capped when centering is below 9", () => {
  const { result } = gradeVintageCache("1989-f-bird-psa4");
  assert.ok(result.psaGrade <= 5);
  assert.ok(
    result.capAudit.some(
...
test("NM guard — moderate+ defects do not bypass uniform_light optimism ceiling", () => {
  const categoryScores = { corners: 9, edges: 8.5, surface: 8.5, centering: 9 };
  const defects = [
    {
      tag: "corner_wear_light",
      severity: "minor",
  ...
  assert.equal(qualifiesForNmUniformLightGuard(8.5, 9, moderateDefects), false);

  const capAudit = [];
  const capped = applyExBandOptimismCeiling(
    8,
    categoryScores,
    defects,
    capAudit,
