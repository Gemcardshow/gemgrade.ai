...
test("formatVerdictDisplay renames projected PSA wording in verdicts", () => {
  const verdict = `## Professional Verdict
Projected PSA 8 based on visible condition. The primary limiter is centering.

### Likely Grade Range
PSA 7–8`;

  const display = formatVerdictDisplay(verdict);

  assert.match(display, /Projected GemGrade 8/);
  assert.match(display, /Estimated Grade Range: 7-8/);
...
test("GEMGRADE_DISCLAIMER is present", () => {
  assert.match(
    GEMGRADE_DISCLAIMER,
    /not affiliated with or endorsed by any third-party grading service/i,
  );
});
