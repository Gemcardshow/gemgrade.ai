  ...
  try {
    ...
    const { grade, deduction, mode: scanMode } = await executeCreditGatedScan({
      ...
      saveScanRecord: async (gradeResult) => {
        const scanSupabase = getServiceRoleClient() ?? getSupabaseClient();
        if (!scanSupabase) {
          return null;
        }

        const { buildScanInsertRecord, insertScanRecord } = await import(
          "./scanHistory.js"
        );

        const insertRecord = buildScanInsertRecord({
          userId: user.id,
          email: scanEmail,
          mode: resolved.mode,
        ...
        const { id: scanId, error: insertError } = await insertScanRecord(
          scanSupabase,
          insertRecord,
        );
        ...
        if (scanId) {
          ...
          const imageResult = await attachScanImagesAfterInsert(scanSupabase, {
            userId: user.id,
            scanId,
            frontImage: gradeFrontImage,
            backImage: gradeBackImage,
          ...
          if (!imageResult.ok) {
            console.error("Scan image persistence failed:", {
              scanId,
              reason: imageResult.reason ?? null,
              storage: imageResult.storage ?? false,
            });
          }
        }
