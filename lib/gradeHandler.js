    ...
    const { grade, deduction, mode: scanMode } = await executeCreditGatedScan({
      ...
      saveScanRecord: async (gradeResult) => {
        ...
        if (scanId) {
          ...
          const imageResult = await attachScanImagesAfterInsert(creditsSupabase, {
            userId: user.id,
            scanId,
            frontImage: gradeFrontImage,
            backImage: gradeBackImage,
          ...
          if (!imageResult.ok) {
            console.error(`Scan image persistence failed scanId=${scanId}`);
            console.error(
              `Scan image persistence reason=${imageResult.reason ?? "unknown"}`,
            );
          }
        }
