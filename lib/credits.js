 ...
 * @param {{ shopifyOrderId?: string | null, appleTransactionId?: string | null }} [options]
...
export async function grantPurchaseCredits(
  ...
  const appleTransactionId =
    typeof options.appleTransactionId === "string" && options.appleTransactionId.trim()
      ? options.appleTransactionId.trim()
      : null;

  if (appleTransactionId) {
    return grantApplePurchaseCredits(
      supabase,
      userId,
      amount,
      metadata,
...
/**
 * Grant Apple consumable credits through the atomic ledger function.
 * The same Apple transaction id cannot increase the balance twice.
 *
 * @param {import("@supabase/supabase-js").SupabaseClient} supabase
 * @param {string} userId
 * @param {number} amount
 * @param {Record<string, unknown>} metadata
 * @param {string} appleTransactionId
 * @returns {Promise<PurchaseGrantResult>}
 */
export async function grantApplePurchaseCredits(
  supabase,
  userId,
  amount,
  metadata,
  ...
  const { data, error } = await supabase.rpc("grant_apple_purchase_credits", {
    p_user_id: userId,
    p_amount: amount,
  ...
  if (error) {
    const message = error.message || "Failed to grant Apple purchase credits";
    const missingFunction =
      error.code === "42883" ||
      error.code === "PGRST202" ||
      /grant_apple_purchase_credits/i.test(message);

    if (missingFunction) {
      throw new PurchaseValidationError(
        "Apple purchase storage is not ready. Apply the Apple IAP migration.",
        503,
      );
    }
  ...
  if (!payload || typeof payload !== "object") {
    throw new Error("Apple purchase grant returned an empty result");
  }

  const creditsGranted = Number(payload.creditsGranted ?? payload.credits_granted ?? 0);
  const balance = Number(payload.balance);

  if (!Number.isFinite(balance)) {
    throw new Error("Apple purchase grant returned an invalid balance");
  }

  return {
    balance,
    transactionId: String(payload.transactionId ?? payload.transaction_id ?? ""),
    creditsGranted: Number.isFinite(creditsGranted) ? creditsGranted : 0,
    alreadyProcessed: creditsGranted === 0,
  };
}
