 ...
 */
export function normalizeScanRowId(scanId) {
  const normalized = String(scanId).trim();
...
export async function uploadScanImageToStorage(supabase, { objectPath, dataUrl }) {
  ...
  const { error } = await supabase.storage
    ...
    .upload(objectPath, new Uint8Array(payload.body), {
...
async function attachLegacyScanImages(supabase, scanId, frontImage, backImage) {
  const rowId = normalizeScanRowId(scanId);
  ...
  return { ok: Boolean(data), reason: data ? null : "legacy_update_no_row" };
}
...
export async function attachScanImagesAfterInsert(
  ...
  const bucketIds = (buckets ?? []).map((bucket) => bucket.id).join(",");
  ...
  if (!frontUpload.ok) {
    ...
    return {
      ok: legacy.ok,
      storage: false,
      reason: `upload:${frontUpload.error ?? "upload_failed"};buckets:${bucketIds || "none"};legacy:${legacy.reason ?? "unknown"}`,
    };
  }
  ...
  if (!data?.front_image_path) {
    ...
    return {
      ok: legacy.ok,
      storage: true,
      reason: `path_update_no_row;legacy:${legacy.reason ?? "unknown"}`,
    };
  }
