...
const SCAN_LIST_SELECT =
  "id, created_at, email, grade, verdict, credits_used, era, confidence, result_snapshot, front_image_path";

const SCAN_DETAIL_SELECT =
  "id, created_at, email, grade, verdict, credits_used, era, confidence, result_snapshot, front_image_path, back_image_path";
...
async function queryUserScans(supabase, filter, options = {}) {
  ...
  if (filter.userId) {
    ...
    if (result.error && isMissingColumnInsertError(result.error)) {
      return { data: scanId ? null : [], error: result.error };
    }
