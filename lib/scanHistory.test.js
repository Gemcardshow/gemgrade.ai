...
import {
  ...
  buildScanThumbnailUrl,
  extractMissingScanColumn,
  fetchUserScanById,
...
test("buildScanInsertRecord includes history metadata without image blobs", () => {
  ...
  assert.equal("front_image" in record, false);
  assert.equal("back_image" in record, false);
});

test("toLegacyScanInsertRecord keeps minimal legacy insert shape", () => {
...
test("mapScanListItem and mapScanDetail shape API payloads", () => {
  ...
  assert.equal(listItem.hasFrontImage, null);
  assert.equal(listItem.frontThumbnailUrl, "/api/scans/scan-1/image?side=front");
  assert.equal(listItem.backImageUrl, "/api/scans/scan-1/image?side=back");

  const detail = mapScanDetail(row);
  assert.equal(detail.result.psaGrade, 8);
  assert.equal(detail.result.credits.deducted, 2);
  assert.equal(detail.frontThumbnailUrl, "/api/scans/scan-1/image?side=front");
});

test("mapScanListItem reports image availability from storage paths", () => {
  const withPaths = mapScanListItem({
    id: "scan-2",
    created_at: "2026-06-10T12:00:00.000Z",
    grade: 8,
    verdict: "Test",
    front_image_path: "user/scan/front.jpg",
    back_image_path: "user/scan/back.jpg",
  });

  assert.equal(withPaths.hasFrontImage, true);
  assert.equal(withPaths.hasBackImage, true);

  const withImage = mapScanListItem({
    id: "scan-2b",
    created_at: "2026-06-10T12:00:00.000Z",
    grade: 8,
    verdict: "Test",
    front_image: "data:image/jpeg;base64,abcd",
  });

  assert.equal(withImage.hasFrontImage, true);
  assert.equal(withImage.frontThumbnailUrl, "/api/scans/scan-2b/image?side=front");

  const withoutImage = mapScanListItem({
    id: "scan-3",
    created_at: "2026-06-10T12:00:00.000Z",
    grade: 8,
    verdict: "Test",
  ...
  assert.equal(withoutImage.hasFrontImage, false);
});

test("parseScanImagePayload decodes data URLs and raw base64", () => {
  const dataUrl = parseScanImagePayload("data:image/jpeg;base64,YWJj");
  assert.ok(dataUrl);
  assert.equal(dataUrl.contentType, "image/jpeg");
  assert.equal(dataUrl.body.toString("utf8"), "abc");

  const raw = parseScanImagePayload("YWJj");
  assert.ok(raw);
  assert.equal(raw.contentType, "image/jpeg");
  assert.equal(raw.body.toString("utf8"), "abc");

  assert.equal(parseScanImagePayload(""), null);
  assert.equal(parseScanImagePayload("   "), null);
});

test("hasScanFrontImage detects saved front images", () => {
  assert.equal(hasScanFrontImage({ front_image: "data:image/jpeg;base64,abc" }), true);
  assert.equal(hasScanFrontImage({ front_image: "" }), false);
  assert.equal(hasScanFrontImage({}), false);
});
...
test("fetchUserScanHistory uses lightweight select and filters by email", async () => {
  const row = {
    id: "scan-1",
    created_at: "2026-06-10T12:00:00.000Z",
    mode: "scout",
  ...
  assert.match(calls.select, /^id, created_at/);
  assert.equal(
    calls.select.split(",").map((column) => column.trim()).includes("front_image"),
    false,
  );
  assert.equal(calls.select.includes("front_image_path"), true);
  assert.equal(calls.email, "test@example.com");
  assert.equal(calls.userId, null);
  assert.equal(calls.limit, 10);
  assert.equal(scans.length, 1);
  assert.equal(scans[0].mode, "scout");
});

test("fetchUserScanHistory filters by user_id when email is unavailable", async () => {
  const row = {
    id: "scan-legacy",
    created_at: "2026-06-10T12:00:00.000Z",
    grade: 8,
...
test("fetchUserScanById uses lightweight select and filters by email", async () => {
  const row = {
    id: "scan-2",
...
test("extractMissingScanColumn parses PostgREST schema errors", () => {
  assert.equal(
    extractMissingScanColumn({
  ...
  assert.equal(extractMissingScanColumn({ message: "duplicate key" }), null);
});

test("insertScanRecord strips missing columns progressively", async () => {
  let attempt = 0;
  const supabase = {
    from() {
      return {
                ...
                async single() {
                  ...
                  if (attempt === 2) {
                    return {
                      ...
                      error: {
                        code: "42703",
                        message: "column scans.mode does not exist",
                      },
                    };
                  }

                  assert.equal(payload[0].user_id, undefined);
                  assert.equal(payload[0].mode, undefined);
                  assert.ok(payload[0].result_snapshot);
                  return { data: { id: "scan-progressive" }, error: null };
                },
              };
            },
          };
  ...
  assert.equal(result.id, "scan-progressive");
});
